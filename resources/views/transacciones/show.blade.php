    ...
    <div class="col-md-8">
        <div class="card">
            <div class="card-header">
                <h5 class="mb-0">
                    <i class="bi bi-coin"></i> Información de la Transacción
                </h5>
            </div>
            <div class="card-body">
                <div class="row mb-3">
                    ...
                    <div class="col-md-8">
                        @if($transaccion->proformas_count > 0)
                            <span class="badge bg-success">
                                <i class="bi bi-file-earmark-text"></i> {{ $transaccion->proformas_count }} proforma(s)
                            </span>
                        @else
        ...
        <div class="card mt-3">
            <div class="card-header">
                <h5 class="mb-0">
                    <i class="bi bi-file-earmark-text"></i> Proformas Asociadas ({{ $transaccion->proformas->count() }})
                </h5>
            </div>
            <div class="card-body">
                @if($transaccion->proformas->count() > 0)
                    <div class="table-responsive">
                        <table class="table table-hover">
                            ...
                            <tbody>
                                @foreach($transaccion->proformas->take(5) as $proforma)
                                <tr>
                                    <td>
                    ...
                    @if($transaccion->proformas->count() > 5)
                        <div class="text-center mt-3">
                            <a href="{{ route('proformas.index') }}?transaccion_id={{ $transaccion->id }}" class="btn btn-sm btn-primary">
                                Ver todas las proformas ({{ $transaccion->proformas->count() }})
                            </a>
                        </div>
                    @endif
                @else
                    <div class="text-center py-4">
                        ...
                        <p class="mt-2 text-muted">Esta transacción no tiene proformas asociadas</p>
                    </div>
                @endif
            </div>
        </div>
            ...
            <div class="card-body">
                ...
                <div class="mt-3">
                    ...
                    <ul class="list-unstyled small">
                        <li><i class="bi bi-check-circle text-success"></i> ID: {{ $transaccion->id }}</li>
                        <li><i class="bi bi-check-circle text-success"></i> Nombre: {{ $transaccion->name }}</li>
                        <li><i class="bi bi-check-circle text-success"></i> Proformas: {{ $transaccion->proformas_count }}</li>
                        @if($transaccion->created_at)
                            <li><i class="bi bi-check-circle text-success"></i> Creado: {{ $transaccion->created_at->format('d/m/Y H:i') }}</li>
                        @endif
                    </ul>
                </div>
            </div>
